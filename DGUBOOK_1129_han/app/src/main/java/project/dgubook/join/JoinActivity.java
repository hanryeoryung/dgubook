...
public class JoinActivity extends AppCompatActivity {
    ...
    EditText email, name, student_id, phone, password1, password2;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        phone = (EditText)findViewById(R.id.join_user_phone);

        Button pushBtn = (Button) findViewById(R.id.push_btn);
        ...
        join_and_confirm.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (email_flag == 1) {
                    if (!email.getText().toString().equals("") && !student_id.getText().toString().equals("") && !phone.getText().toString().equals("") && !name.getText().toString().equals("") && !password1.getText().toString().equals("") && !password2.getText().toString().equals("")) {
                        //빈칸없는지 확인
...
//                    builder.setMessage("빈칸없이 기입해 주세요!").show();
    ...
    private void toaddPush() {
        if (email.getText().toString().equals("")){
            Toast.makeText(JoinActivity.this, "이메일을 입력해주세요", Toast.LENGTH_SHORT).show();
        }else{
            Intent tmp = new Intent(Intent.ACTION_VIEW);
            tmp.setClassName(JoinActivity.this, PushTest.class.getName());
            tmp.putExtra("email", email.getText().toString());
            startActivity(tmp);
        }
    }
